...
def llm_classify(text, question, choices=None, freeform=False):
    ...
    for _ in range(3):
        try:
            ...
            err = str(e)
            # Retry on RESOURCE_EXHAUSTED, 503/UNAVAILABLE errors
            if "RESOURCE_EXHAUSTED" in err or "503" in err or "UNAVAILABLE" in err:
                time.sleep(15)
                continue
            else:
    ...
    # If all 3 attempts fail with retryable messages, return empty string
    return ""
...
for pdf_path in pdf_paths:
    ...
    # 1) Bug detection relevance (Yes/No)
    ...
    # 7) Approach / technical contribution
    ...
    # 12) Benchmark source (renamed from 'benchmark_origin')
...
# Reorder columns to match RAW_KEYS exactly
_df = df[RAW_KEYS]
